...
/**
 * AnimatedCard — 21st.dev's data-visualisation card with on-hover layered
 * animations (bar chart slides, gradient wipes, pills fade, header reveals).
 *
 * Imported as-is for future dashboard widgets. For admin GRID cards we use
 * `components/admin/AdminCard.tsx` instead, which mirrors the same
 * group/animated-card hover trick on simpler iconographic content.
 *
 * NOTE: the original 21st.dev snippet re-declared its own `cn` helper. This
 * project already has one at `@/lib/cn`; importing from there avoids two
 * copies of the same util.
 */
